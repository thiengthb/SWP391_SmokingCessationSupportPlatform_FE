...
import { format, differenceInDays, addDays, isFuture, isPast, isToday } from 'date-fns';
...
import type { QuitPlan, Phase } from '../index';
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {format(plan.targetQuitDate, 'EEEE, dd/MM/yyyy', { locale: vi })}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start">
                  <Calendar
            ...
            <div className="flex flex-col gap-3">
              ...
              {currentPhase && (
                <div className="flex items-center gap-3 p-3 rounded-md bg-muted/50">
                  <div className={`rounded-full p-2 ${currentPhase.completed ? 'bg-green-100' : 'bg-primary/10'}`}>
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      Giai đoạn hiện tại: {currentPhase.targetCigarettes} điếu/ngày
                    </div>
                  </div>
                </div>
              )}
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col bg-muted/30 rounded-lg p-3">
                <div className="text-sm text-muted-foreground mb-1">Thời gian kế hoạch</div>
                <div className="font-semibold">{daysInPlan} ngày</div>
              </div>
              
              <div className="flex flex-col bg-muted/30 rounded-lg p-3">
                <div className="text-sm text-muted-foreground mb-1">Điếu thuốc không hút</div>
                <div className="font-semibold">{savedCigarettes} điếu</div>
              </div>
              
              <div className="flex flex-col bg-muted/30 rounded-lg p-3">
                <div className="text-sm text-muted-foreground mb-1">Tiền tiết kiệm</div>
                <div className="font-semibold">{new Intl.NumberFormat('vi-VN').format(moneySaved)} ₫</div>
              </div>
              
              <div className="flex flex-col bg-muted/30 rounded-lg p-3">
                <div className="text-sm text-muted-foreground mb-1">Số giai đoạn</div>
                <div className="font-semibold">{plan.phases.length} giai đoạn</div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center space-x-4">
              ...
              <div className="flex flex-col border-l pl-4 max-w-[180px]">
                <p className="text-sm">
                  Mỗi ngày bỏ thuốc giúp cơ thể bạn phục hồi dần và giảm nguy cơ các bệnh tim mạch, phổi.
                </p>
              </div>
            </div>
          </CardContent>
